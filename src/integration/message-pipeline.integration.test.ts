 ...
 *
 * 测试完整的消息流程:
 *   MessageRouter → CentralController → AgentRuntime → AgentBridge/LightLLMClient → ResponseDispatcher
 *
...
describe('消息管道集成测试', () => {
  ...
  describe('端到端: MessageRouter → CentralController → AgentRuntime', () => {
    test('复杂任务应该经过 Claude Bridge 处理并通过 dispatcher 返回', async () => {
      const agentBridge = createMockAgentBridge('这是复杂任务的回答');
      ...
      expect((agentBridge.execute as ReturnType<typeof mock>).mock.calls.length).toBe(1);
    ...
    test('简单任务应该经过 LightLLM 处理', async () => {
      ...
      expect((agentBridge.execute as ReturnType<typeof mock>).mock.calls.length).toBe(0);
    ...
    test('LightLLM 不可用时，简单任务应该回退到 Claude Bridge', async () => {
      const agentBridge = createMockAgentBridge('fallback 回答');
  ...
  describe('会话历史维护', () => {
    test('连续两条消息应该累积到同一会话', async () => {
      ...
      const agentBridge = createMockAgentBridge('回答1');
    ...
    test('不同 conversationId 应该隔离到不同会话', async () => {
      const sessionManager = new SessionManager();
      const agentBridge = createMockAgentBridge('reply');
  ...
  describe('错误处理管道', () => {
    ...
    test('分类器失败时应该保守默认为 complex 并继续', async () => {
      const agentBridge = createMockAgentBridge('safe fallback');
  ...
  describe('ResponseDispatcher 集成', () => {
    test('无 dispatcher 时不应崩溃', async () => {
      const agentBridge = createMockAgentBridge('no dispatcher');
